...
const Post = ({ post }) => {
  ...
  useEffect(() => {
    ...
    fetchComments()
  }, [post._id, token])  // token added here
      ...
      <div className={classes.wrapper}>
        <div className={classes.top}>
          <Link to={`/profileDetail/${post?.user?._id}`} className={classes.topLeft}>
            <img
              src={woman}
              alt={post?.user?.username ? `${post.user.username} profile` : 'User profile'}
        ...
        <div className={classes.center}>
          ...
          <img
            ...
            src={post?.photo ? `http://localhost:5000/images/${post?.photo}` : woman}
            alt={post?.desc ? post.desc : 'Post image'}
          />
        </div>
